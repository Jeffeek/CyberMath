...
namespace CyberMath.Structures.BinaryTrees.BinaryTreeBase
{
		...
		where T : IComparable, IComparable<T>
	{
		...
		public IEnumerable<T> Preorder() =>
			ReferenceEquals(Root, null) ? Enumerable.Empty<T>() : InternalPreorder(Root);
		...
		public IEnumerable<T> Postorder() =>
			ReferenceEquals(Root, null) ? Enumerable.Empty<T>() : InternalPostorder(Root);
		...
		private IEnumerable<T> InternalPostorder(IBinaryTreeNode<T> node)
		{
			...
			while (stack.Count > 0)
			{
				...
				if (prev == null ||
				    ReferenceEquals(prev.Left, current) ||
