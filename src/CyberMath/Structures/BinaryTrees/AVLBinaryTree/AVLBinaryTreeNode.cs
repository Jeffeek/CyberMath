...
namespace CyberMath.Structures.BinaryTrees.AVLBinaryTree
{
		...
		where T : IComparable<T>, IComparable
	{
		...
		private AVLBinaryTreeNode<T> InternalRemove(AVLBinaryTreeNode<T> node, T value)
		{
			...
			switch (value.CompareTo(node.Data))
			{
				...
				default:
				{
					...
					if (min == null)
						throw new ArgumentNullException(nameof(min) + "node was null, when it's impossible");
		...
		{
			var leftNode = GetHeight(node.Left as AVLBinaryTreeNode<T>);
			var rightNode = GetHeight(node.Right as AVLBinaryTreeNode<T>);
			node._height = (leftNode > rightNode ? leftNode : rightNode) + 1;
		}
