...
namespace CyberMath.Structures.Matrices.Matrix
{
	...
	public class Matrix<T> : IMatrix<T>, IEquatable<Matrix<T>>
	{
		...
		public IMatrix<T> Transpose()
		{
			...
			{
				for (var j = 0; j < ColumnsCount; j++) result[j, i] = _innerMatrix[i, j];
			}
		...
		private IMatrix<T> PrimitiveClone()
		{
			...
			for (var i = 0; i < RowsCount; i++)
			{
				for (var j = 0; j < ColumnsCount; j++) clone[i, j] = this[i, j];
			}
		...
		private IMatrix<T> SerializableClone()
		{
			...
			for (var i = 0; i < RowsCount; i++)
			{
				for (var j = 0; j < ColumnsCount; j++) clone[i, j] = this[i, j].SerializableDeepCopy();
			}
