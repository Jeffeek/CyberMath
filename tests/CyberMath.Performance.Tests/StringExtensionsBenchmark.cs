...
public class StringExtensionsBenchmark
{
    ...
    [Benchmark]
    public char[] ToAlternatingCaseOverKill()
    {
        var length = S.Length;
    ...
    private static char ShiftCase(char c)
    {
        ...
        return i switch
        {
            < 65 or > 122 => c,
            < 97 and > 90 => c,
            _ => (char)(i > 90 ? c & ~0x20 : c | 0x20)
        };
    }

    [Benchmark]
    public void ToAlternatingCaseOverLinq() => string.Concat(S.Select(c => (char)(c ^ c / 65 * (1 - (c - 1) % 32 / 26) * 32)));

    [Benchmark]
    public void ToAlternatingCaseLinq() => string.Concat(S.Select(c => char.IsUpper(c) ? char.ToLower(c) : char.ToUpper(c)));
}