...
public class Int32ExtensionTests
{
    ...
    public void IsOdd_ReturnsExpectedResult(int number, bool expected)
    {
    ...
    public void LCM_ThrowsOnOverflow()
    {
        ...
        Assert.Throws<OverflowException>(() => int.MaxValue.LCM(2));
    }

    [Theory]
    [InlineData(1000000, 2000000)]  // Large but safe values
    [InlineData(46340, 46341)]      // Near sqrt(int.MaxValue)
    public void LCMSafe_HandlesLargeValues(int a, int b)
    {
        var result = a.LCMSafe(b);
        Assert.True(result > 0 || (a == 0 || b == 0));
    }
    ...
    public void Swap_SwapsValues(int initial_a, int initial_b)
    {
        int a = initial_a;
        int b = initial_b;
        ...
        Assert.Equal(initial_b, a);
        Assert.Equal(initial_a, b);
    }
    ...
    public void IsPalindrome_ReturnsExpectedResult(int number, bool expected)
    {
    ...
    public void GetLength_ReturnsCorrectLength(int number, int expectedLength)
    {
    ...
    public void GetDigits_ReturnsCorrectDigits(int number, byte[] expected)
    {
    ...
    public void ToBinary_ReturnsCorrectBinaryString(int number, string expected)
    {
    ...
    public void ToHex_ReturnsCorrectHexString(int number, string expected)
    {
...
public class Int32PrimeNumbersTests
{
    [Theory]
    [InlineData(2, true)]
    [InlineData(3, true)]
    ...
    {
        var primes = Int32PrimeNumbers.GeneratePrimeNumbers().Take(10).ToList();
