...
public class RedBlackBinaryTreeTests
{
    [Fact]
    public void BinaryTreeMergeTest()
    {
        ...
        var checkArray = Enumerable.Range(0, 20).ToArray();
        Assert.Equal(checkArray, firstTree.Inorder().ToArray());
    }
    ...
    public void BinaryTreeMergeTime_100_Test()
    {
        ...
        firstTree.MergeWith(secondTree);
        Assert.Equal(list, firstTree.Inorder().ToArray());
    }
    ...
    {
        var rnd = new Random();
        var numbers = Enumerable.Range(0, 100).Select(_ => rnd.Next(-30, 30)).Distinct().ToArray();
    ...
    {
        var rnd = new Random();
        var numbers = Enumerable.Range(0, 1000).Select(_ => rnd.Next(-30, 30)).Distinct().ToArray();
        var tree = new RedBlackBinaryTree<int>(numbers);
        var sortedNumbers = numbers.ToList();
        sortedNumbers.Sort();
        var inorder = tree.Inorder().ToArray();
    ...
    {
        var rnd = new Random();
        var numbers = Enumerable.Range(0, 10000).Select(_ => rnd.Next(-30, 30)).Distinct().ToArray();
        var tree = new RedBlackBinaryTree<int>(numbers);
        var sortedNumbers = numbers.ToList();
        sortedNumbers.Sort();
        var inorder = tree.Inorder().ToArray();
    ...
    public void BinaryTree_RemoveTest()
    {
        ...
        for (var i = 0; i < 10; i++)
            tree.Add(i);
        tree.Remove(5);
        Assert.Equal(new[] { 0, 1, 2, 3, 4, 6, 7, 8, 9 }, tree.Inorder().ToArray());
    }
    ...
    {
        var rnd = new Random();
        var listOfNums = Enumerable.Range(0, 1000).Select(_ => rnd.Next(-10_000, 10_000)).ToArray();
        var tree = new RedBlackBinaryTree<int>();
